...
// brand matches this site's accent (--color-accent-from) and background (--color-bg)
const THEMES = {
  default: {
    bg: '#161618',
...
type GlobalColor = keyof typeof THEMES
...
export function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      const theme = THEMES[globalColor] ?? THEMES.brand
  ...
  useEffect(() => {
    ...
    // Users who prefer reduced motion get a static grid: no pointer warp, no
    // click ripples, no animation loop.
  ...
  }, [animate, draw, prefersReducedMotion])
